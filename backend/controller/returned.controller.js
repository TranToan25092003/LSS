...
module.exports.borrowerConfirmReturn = async (req, res) => {
    ...
    try {
        ...
        if (existingReturn) {
            ...
            return res.status(200).json({ message: "Return status updated", data: existingReturn });
        } else {
            ...
            res.status(201).json({ message: "Return created and confirmed", data: newReturn });
        }
    } catch (err) {
...
module.exports.ownerConfirmReturn = async (req, res) => {
    ...
    try {
        ...
        returnRecord.overTime = confirmStatus ? Math.round(overtime * 100) / 100 : 0; // Nếu xác nhận, tính overtime
        await returnRecord.save();
        ...
        res.status(200).json({
            message: confirmStatus ? "Owner confirmed return" : "Owner undone confirm",
            data: {
                returnRecord,
                item,
            },
...
module.exports.getAllReturns = async (req, res) => {
    ...
    try {
        ...
        // Kiểm tra nếu người dùng là người mượn (borrower)
        const borrowRecords = await Borrow.find({ borrowerClerkId: userId })
            .populate("item")  // Lấy thông tin item từ bảng Item
            .sort({ createdAt: -1 });

        // Nếu người dùng là người cho mượn (owner)
        const lendRecords = await Lend.find({ "item.ownerClerkId": userId })
            .populate("item")  // Lấy thông tin item từ bảng Item
            .sort({ createdAt: -1 });
        console.log(userId);
        ...
        const returnData = await Promise.all(allRecords.map(async (record) => {
            ...
            const borrowerConfirm = returnRecord ? returnRecord.borrowerConfirm : false;
            ...
            return {
                ...
                itemStatus: record.item.status,
                timeReturned: returnRecord ? returnRecord.timeReturned : "Not Returned",  // Nếu có Return thì tính thời gian trả lại
                overTime: returnRecord ? returnRecord.overTime : 0,  // Nếu có Return thì tính overtime
