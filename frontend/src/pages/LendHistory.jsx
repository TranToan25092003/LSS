...
const LendHistory = () => {
  ...
  useEffect(() => {
    const fetchLendHistory = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await customFetch.get(`/lends/history/${user.id}`);
        setLendHistory(response.data.data);
      } catch (error) {
        console.error("Error fetching lend history:", error);
        setError(
          error.response?.data?.message || "Lỗi khi tải lịch sử cho mượn"
        );
      } finally {
        setLoading(false);
      }
  ...
  if (!lendHistory || lendHistory.length === 0) {
    return (
      <div className="container mx-auto py-8 text-center">
        <HistoryButtons />
        <h1 className="text-2xl font-bold mb-6">Lịch sử cho mượn</h1>
        <p className="text-gray-500">Bạn chưa có lịch sử cho mượn nào</p>
        <button
          onClick={() => navigate("/")}
          className="mt-4 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
        >
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead>Thời gian cho mượn</TableHead>
